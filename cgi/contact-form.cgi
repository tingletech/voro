...
#------------------------------------------------------------------------
# CDL Revision History
#	mostly unknown
#	2008/10/20 - MAR - Handle the e-mail differently, if the content is
#		more than three non-CDL URLs.  (Most of our spam fits
#		that criterion.)
#------------------------------------------------------------------------
...
BEGIN
{
  ...
  $postmaster        = '';
  @referers          = qw(calisphere.universityofcalifornia.edu);
  push(@referers, $ENV{'PUBLICDL_HOSTNAME'});
  @allow_mail_to     = qw(CALI-L@LISTSERV.UCOP.EDU);
  #@allow_mail_to     = qw(esatzman@ucop.edu);
  @recipients        = ();
  %recipient_alias   = ();
  @valid_ENV         = qw(REMOTE_HOST REMOTE_ADDR REMOTE_USER HTTP_USER_AGENT HTTP_REFERER);
  $locale            = '';
  $charset           = 'iso-8859-1';
...
#  $self->validate_form or return;	# Ravi Gudipati 10/23/03
	$validate_rc = $self->validate_form;
	return if ($validate_rc == 0);

	# Set the "don't send" flag, based on the return code.
	if ($validate_rc < 0) {
		$dont_send = 1;
		}
	else {
...
Validates the form fields.  If it fails, it outputs an error page, and
returns 0 (false).  Under two conditions, it returns -1 to allow the
caller to act as if the mail has been sent.  They are (1)  if it finds
more than 3 non-CDL URLS in the "Comments:" area, or (2)  if the referer URL
does not end with "/contact-us-faq.html".  If all is OK, it returns 1 (true).
...
sub validate_form {
  my ($self) = @_;
	...
	my $cmts = $self->cgi_object->param("Comments");
	my $emailStr = $self->cgi_object->param("Email");
	my $name = $self->cgi_object->param("Name");
	# my $ccUser = $self->cgi_object->param("CcUser");
	# my $bccUser = $self->cgi_object->param("BccUser");

	# 2008/10/20 - MAR - If the "Comments:" field contains at least three
	#	non-CDL URLs, then don't send the form, but don't let the
	#	user know that her e-mail went to the bit bucket.
	my $work_comments = $cmts;
	my $the_url;
	my $non_cdl_url_count = 0;
	while ($work_comments =~ m|https?://([-a-z0-9._]+)(.*)$|si) {
		$the_url = $1;
		$work_comments = $2;

		# Allow CDL URLs.
		next if ($the_url =~ /\.cdlib\.org$/i);
		next if ($the_url =~ /\.universityofcalifonia.edu$/i);

		# This is a non-CDL URL.
		$non_cdl_url_count++;
		}

	if ($non_cdl_url_count >= 3) {
		$self->printLog($emailStr . " (>=3 non-CDL URLs)", $name);
		return -1;
		}

	# 2008/10/20 - MAR - If the referrer URL does not end with
	# "/contact-us-faq.html", disallow the mail.
	my $referrer = $ENV{HTTP_REFERER};
	if ((! defined($referrer)) || ($referrer !~ /\/contact-us-faq.html$/)) {
		$self->printLog($emailStr . " (wrong referrer URL:  " .
			(defined($referrer) ? "$referrer)" : "not defined)"),
			$name);
		return -1;
		}

	if ($cmts eq "") {
		 $self->error_page( 'Error: Required fields', <<END );
<p align="center">
  The form requires some <strong>comments</strong>, please enter some <strong>comments</strong> and click submit again.
</p>
END
		$self->printLog($emailStr, $name);
		return 0;
	}

	# 2008/10/20 - MAR - Getting error "Use of implicit split to @_ is
	# deprecated at (eval 5) line 629." on the next line.
#	if ( (scalar(split(/,/, $emailStr)) > 1) ) {
	if (index($emailStr, ",") >= 0) {
		 $self->error_page( 'Error: Email limit exceeded', <<END );
<p>
  You can't enter more than one email, please enter one email address and click submit again.
</p>
END
	...
	if ($emailStr =~ /\s/)  {
		 $self->error_page( 'Error: Invalid Email', <<END );
<p>
  You can't enter invalid email, please enter correct email address and click submit again.
</p>
END
		$self->printLog($emailStr, $name);
		return 0;
...
#Ravi Gudipati - 05/01/2006
sub printLog {
	# my $self = shift;
	# my $from = shift;
	# my $name = shift;
# 
 	# open(OUT, ">> /publicdl/local/logs/formmail.log") || die "Can't open file, $!\n";
 # $| = 1;
# 
...
# Ravi G - 10/22/03
#push @{ $self->{Recipients} }, $self->cgi_object->param('CcUser') if ($self->cgi_object->param('CcUser') );
#push @{ $self->{Recipients} }, $self->cgi_object->param('BccUser') if ($self->cgi_object->param('BccUser') );
